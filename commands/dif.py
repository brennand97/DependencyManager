    ...
    if not len(arg_lst) == 2:
        ...
        for arg in arg_lst:
            if len(arg) == 2 and arg[0] == "--sql":
                ...
                print("Invalid syntax: dif [--sql] <filepath1> [--sql] <filepath2>")
                return
    f1_new, f1_dep = data.load_sql_tmp(files[0][0]) if files[0][1] else data.load_tmp(files[0][0])
    f2_new, f2_dep = data.load_sql_tmp(files[1][0]) if files[1][1] else data.load_tmp(files[1][0])
